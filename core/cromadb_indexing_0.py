...
healthfood_claims_path = (
    "csv_data/healthfood_claims_final10.csv"  # 새 CSV 파일 경로 추가
)
...
try:
    ...
    print(
        f"❌ 오류: {healthfood_claims_path} 파일을 찾을 수 없습니다. 이 파일에 대한 처리를 건너뜁니다."
    )
    df_healthfood_claims = (
        pd.DataFrame()
    )  # 빈 데이터프레임으로 초기화하여 이후 로직에서 오류 방지
...
def healthfood_claims_to_doc(row):
    ...
    if "밀크씨슬" in product_name or "밀크씨슬" in functionality_content:
        ...
        print(
            f"  > 원본 행 데이터 (일부): 제품명='{product_name}', 기능성내용='{functionality_content}'"
        )
    ...
    if (
        not product_name or not functionality_content
    ):  # 제품명과 기능성 내용이 없으면 유효하지 않은 문서로 간주
        return None
...
docs_healthfood_claims = [
    healthfood_claims_to_doc(row) for _, row in df_healthfood_claims.iterrows()
]
...
for i in range(0, num_docs, batch_size):
    ...
    print(
        f"  ➡️ 배치 {i//batch_size + 1}: {len(batch_docs)}개 문서 처리 중 ({i + len(batch_docs)}/{num_docs})..."
    )
    try:
        vector_db.add_documents(
            documents=batch_docs
        )  # LangChain Chroma는 documents 매개변수명을 사용합니다.
    except Exception as e:
        print(f"  ❌ 오류 발생! 배치 {i//batch_size + 1} 처리 중 문제 발생: {e}")
        print(
            f"    해당 배치의 첫 번째 문서 (일부): {str(batch_docs[0])[:200] if batch_docs else '없음'}"
        )
    ...
    try:
        ...
        if vector_db._collection is not None:
            print(
                f"✨ ChromaDB 컬렉션 '{vector_db._collection.name}'에 현재 {vector_db._collection.count()}개의 문서가 저장되어 있습니다."
            )
        else:
...
## index 상황 : e알약은, 기능성데이터, healthfood 데이터 정제(솔님)
## 실제 사용 : 기능성 데이터, healthfood 데이터 정제 사용 중
