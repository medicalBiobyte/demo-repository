...
from matplotlib import font_manager, rc  # font_manager와 rc 임포트
...
# --- 데이터 로드 (실제 파일 경로에 맞춰 수정 필요) ---
# 예시: 파일이 현재 스크립트와 같은 디렉토리에 있다고 가정
# 만약 파일이 없다면 이 부분에서 에러가 발생할 수 있습니다.
try:
    df_healthfood = pd.read_csv(healthfood_claims_path)
    df = df_healthfood.copy()
except FileNotFoundError:
    print(
        f"Error: {healthfood_claims_path} 파일을 찾을 수 없습니다. 파일 경로를 확인해주세요."
    )
    exit()  # 파일 없으면 프로그램 종료
...
# --- Matplotlib 및 WordCloud 한글 폰트 설정 ---
# Windows 환경에서 Malgun Gothic 폰트 경로를 시도합니다.
# 만약 Malgun Gothic이 없거나 다른 폰트를 사용하고 싶다면,
# 해당 폰트 파일의 정확한 경로로 변경해주세요.
# 예: 'C:/Windows/Fonts/NanumGothic.ttf' 또는 'C:/Windows/Fonts/malgun.ttf'
font_path = "C:/Windows/Fonts/malgunbd.ttf"  # 맑은 고딕 볼드체 (일반체는 malgun.ttf)

# 폰트 경로 유효성 검사
if not os.path.exists(font_path):
    print(f"경고: 지정된 폰트 파일 '{font_path}'을(를) 찾을 수 없습니다.")
    print("한글이 깨져 보이거나 오류가 발생할 수 있습니다.")
    print("시스템에 한글 폰트(예: 맑은 고딕, 나눔 고딕)가 설치되어 있는지 확인하고,")
    print("위 'font_path' 변수의 경로를 올바르게 수정해주세요.")
    # 대체 폰트 또는 기본 폰트로 진행 (한글 깨짐 발생 가능성 있음)
    # plt.rc("font", family="DejaVu Sans")
    # font_path = None # WordCloud에서 폰트 지정을 안 하도록
else:
    # Matplotlib 폰트 설정
    font_name = font_manager.FontProperties(fname=font_path).get_name()
    rc("font", family=font_name)
    plt.rcParams["axes.unicode_minus"] = False  # 마이너스 폰트 깨짐 방지
    print(f"Matplotlib 및 WordCloud에 '{font_name}' 폰트 적용 완료.")
...
for kws_list in df["matched_keywords"]:
    if isinstance(kws_list, list):  # 리스트 타입인지 확인
        all_kws.extend(kws_list)

overall_freq = Counter(all_kws)

# 폰트 경로가 유효할 경우에만 WordCloud 생성
if os.path.exists(font_path):
    wc_all = WordCloud(
        font_path=font_path,  # 설정된 한글 폰트 경로 사용
        width=800,
        height=600,
        background_color="white",
        max_font_size=180,
        colormap="PuBuGn",  # 워드클라우드 색상 맵
        random_state=42,
    ...
    print("워드클라우드 생성을 건너뜁니다. 폰트 파일을 찾을 수 없습니다.")
